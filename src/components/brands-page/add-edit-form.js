...
const EditAddBrandForm = ({ isEditForm, loading, data = {} }) => {
  ...
  const submitHandler = async (values) => {
    ...
    try {
      if (isEditForm) {
        await AxiosInstance.put(`/brand/update/${data._id}`, values);
        toast.success(intl.formatMessage({ id: "brand updated" }));
      } else {
        const {
          data: { _id },
        } = await AxiosInstance.post(`/brand/create`, values);
        toast.success(intl.formatMessage({ id: "brand added" }));
        history.replace(`/dashboard/brands/${_id}/edit`);
      }
    } catch (error) {
      console.log(error);
    }
            ...
            <div>
              <CFormGroup>
                ...
                <CInput
                  ...
                  type="text"
                  name="brandName[ar]"
                  defaultValue={data?.brandName?.ar}
                  invalid={!!errors.brandName?.ar?.message}
                  innerRef={register({
                    required: intl.formatMessage({ id: "required field" }),
                  })}
                />
                <CInvalidFeedback>
                  {errors.brandName?.ar?.message}
                </CInvalidFeedback>
              </CFormGroup>
              {/* English Name */}
              ...
              <CFormGroup>
                ...
                <CInput
                  ...
                  type="text"
                  name="brandName[en]"
                  defaultValue={data?.brandName?.en}
                  invalid={!!errors.brandName?.en?.message}
                  innerRef={register({
                    required: intl.formatMessage({ id: "required field" }),
                  })}
                />
                <CInvalidFeedback>
                  {errors.brandName?.en?.message}
                </CInvalidFeedback>
              </CFormGroup>
...
export default EditAddBrandForm;
